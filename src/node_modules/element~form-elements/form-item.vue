...
<template>
  ...
  <div class="formItem {{variantClass}}">
    ...
    <input class="" :class="{ 'input-invalid': !validation.valid }" type="{{type}}" @keyup.enter="inputEnter" required id="{{inputId}}" name="{{inputId}}" value="{{value}}" v-on:input="input()" v-on:blur="blur()" valid="{{validRegex}}" v-model="value" placeholder="{{placeholder}}" :readonly="readonly"/><a v-if="readonly" v-on:click="edit" class="readonly_edit">ändern</a>
...
<script>
  ...
  module.exports = {
    props: ['variant', 'label', 'validationType', 'validationMessage', 'test', 'regexp', 'min', 'max', 'inputId', 'defaultValue', 'type', 'placeholder', 'readonly' ],
    data: function() {
      return  {
        validation: {
          valid: true,
    ...
    methods: {
        ...
        if(!Array.isArray(validationTypes) || !Array.isArray(validationTypes)) {
          ...
          for ( var i = 0; i < validationTypes.length; i++) {
            if(!this.validation.methods[validationTypes[i]].call(this)){
              ...
              this.$dispatch('value-change', this.value, this.validation.valid);
              return false;
            } else {
      ...
      inputEnter: function(){
        ...
        this.validate();
        this.$dispatch('input-enter', this.inputId);
      }
    }
  }
</script>