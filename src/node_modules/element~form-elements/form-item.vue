...
<template>
  ...
  <div class="formItem {{variantClass}}">
    ...
    <input class="" :class="{ 'input-invalid': !validation.valid }" type="{{type}}" required id="{{inputId}}" name="{{inputId}}" value="{{value}}" v-on:input="input()" v-on:blur="blur()" valid="{{validRegex}}" v-model="value" placeholder="{{placeholder}}" :readonly="readonly"/><span v-if="readonly" v-on:click="edit" class="icon icon-pencil" style="font-size:19px;"></span>
