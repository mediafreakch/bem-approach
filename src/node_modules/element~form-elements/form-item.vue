...
<template>
  ...
  <div class="formItem {{variantClass}}">
    ...
    <input class="" :class="{ 'input-invalid': !validation.valid }" type="{{type}}" @keydown.enter="inputEnter" required id="{{inputId}}" name="{{inputId}}" value="{{value}}" @input="input()" @blur="blur()" valid="{{validRegex}}" v-model="value" placeholder="{{placeholder}}" :readonly="readonly"/><a v-if="readonly && editable" @click="edit" class="readonly_edit">ändern</a><span v-if="!readonly" class="icon icon-spinner" :class="{ 'animated': retrievingData}"></span>
  ...
  module.exports = {
    ...
    methods: {
      blur: function(){
        ...
        this.$dispatch('input-blur', this.inputId, this.value);
      },
      input: function(){
        if(this.validation.state === 'pristine') {
          return;
        ...
        if(!Array.isArray(validationTypes) || !Array.isArray(validationTypes)) {
          ...
          for ( var i = 0; i < validationTypes.length; i++) {
            if(!this.validation.methods[validationTypes[i]].call(this)){
              ...
              this.$dispatch('value-change', this.inputId, this.value, this.validation.valid);
              return false;
            } else {
      ...
      edit: function(){
        this.$dispatch('input-edit', this.inputId);
      },
      inputEnter: function(){
        if(this.validation.state === 'pristine'){
          this.validation.state = 'dirty';
        ...
        this.validate();
        this.$dispatch('input-enter', this.inputId, this.value, this.validation.valid );
      }
    }
  }
</script>